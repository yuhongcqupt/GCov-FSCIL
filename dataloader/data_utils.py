...
from dataloader.sampler import CategoriesSampler_train
from dataloader.sampler import CategoriesSampler_test
from dataloader.sampler import CategoriesSampler_train_preudo
from dataloader.sampler import CategoriesSampler_test_preudo

from dataloader.sampler import CategoriesSupportSampler
from dataloader.sampler import CategoriesSupportSampler_test
...
def get_base_dataloader_gen(args):
    txt_path = "data/index_list/" + args.dataset + "/session_" + str(0 + 1) + '.txt'
    class_index = np.arange(args.base_class)
    if args.dataset == 'cifar100':
        trainset = args.Dataset.CIFAR100(root=args.dataroot, train=True, download=True,
    ...
    sampler = CategoriesSampler_train(trainset.targets, 1, 10, 20, all_cls=100)
    sampler_ = CategoriesSampler_test(testset.targets, 1, 10, 20, all_cls=100)
    trainloader = torch.utils.data.DataLoader(dataset=trainset, batch_sampler=sampler, num_workers=args.num_workers,
                                              pin_memory=True)

    testloader = torch.utils.data.DataLoader(dataset=testset, batch_sampler=sampler_, num_workers=args.num_workers,
                                              pin_memory=True)

    return trainloader, testloader


def get_support_dataloader_gen(args):
    txt_path = "data/index_list/" + args.dataset + "/session_" + str(0 + 1) + '.txt'
    class_index = np.arange(args.base_class)
    if args.dataset == 'cifar100':
        trainset = args.Dataset.CIFAR100(root=args.dataroot, train=True, download=True,
    ...
    sampler = CategoriesSupportSampler(trainset.targets, 1, 90, 2, all_cls=100)
    trainloader = torch.utils.data.DataLoader(dataset=trainset, batch_sampler=sampler, num_workers=args.num_workers,
                                              pin_memory=True)
